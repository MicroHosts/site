...
export default async function handler(
    ...
    const ticket = vncproxy.ticket
    setCookie("PVEAuthCookie",encodeURI(`${ticket}`), {req, res})
    ...
    res.status(200).json({ message: `https://82.140.114.122:8080/?console=shell&novnc=1&node=main3&resize=scale&vmid=${host.vimid}&path=${encodeURI(`api2/json/nodes/main3/qemu/${host.vimid}/vncwebsocket?port=${vncproxy.port}&vncticket=${vncproxy.ticket}`)}` });
    return
}
